...
async def hunter(domain, key):
    try:
        r=requests.get("https://api.hunter.io/v2/domain-search", params={"domain":domain,"api_key":key}, timeout=5)
...
async def search_and_scrape(keyword, location, max_sites, hunter_key, engine="ddg", bing_key=""):
    ...
    urls = ddg_search(query,max_sites) if engine=="ddg" else bing_search(query,max_sites,bing_key)
